...
if [[ $REPLY =~ ^[Yy]$ ]] && [ -d "/sys/firmware/efi/efivars" ]; then
    ...
    umount /root/btrfs
    mount -o "subvol=subvol_os1_fsroot,rw,noatime,compress-force=zstd:3,noautodefrag" $2 /mnt
    mount -m -o "subvol=subvol_os1_var_cache_pacman_pkg,rw,noatime,compress-force=zstd:3,noautodefrag" $2 /mnt/var/cache/pacman/pkg
    mount -m $1 /mnt/boot
