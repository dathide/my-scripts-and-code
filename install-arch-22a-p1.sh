...
if [[ $REPLY =~ ^[Yy]$ ]] && [ -d "/sys/firmware/efi/efivars" ]; then
    ...
    btrfs subvolume create /root/btrfs/subvol_fsroot
    btrfs subvolume create /root/btrfs/subvol_var
    umount /root/btrfs
    mount -o "subvol=subvol_fsroot,rw,noatime,compress-force=zstd:3,noautodefrag" $2 /mnt
    mount --mkdir -o "subvol=subvol_var,rw,noatime,compress-force=zstd:3,noautodefrag" $2 /mnt/var
